...
The OnTriggerEnter method is called when the GameObject with this script, the player, enters a trigger collider. If the collided object has the tag "Key", the player has collected a key. Then it increases keysCollected by 1, logs the new total, shows the keysFound text and updates it to display the current key count. Afterward it removes the key object from the scene to prevent re-collection. If the player now has enough keys (keysCollected >= keysNeeded), it calls OpenGate() to open the gate.
